...
c
c  Developed originally in about 2005 by arnie lee van buren and jeffrey
c  boisvert. Updated several times since then. For more information see:
c  GitHub.com/MathieuandSpheroidalWaveFunctions/Mathieu
c
...
c  in real arithmetic. Procedures used in matfcn allow for exponents
c  much larger than nex since the resulting floating point radial
c  function values are given as a characteristic and an integer
c  exponent. Testing of matfcn shows that in real*8 arithmetic, it
c  provides good results for q positive for virtually all values of xi
c  and for c up to at least 10000. It also provides good results for q
c  negative for virtually all values of xi down to 1.000005 or less
c  and for c up to at least 10000i. See the discussion of accuracy
c  below.
c
c  Matfcn can be run in either double precision or quadruple precision
c  arithmetic. The choice is set in the module param located above at
c  the beginning of matfcn. Here, the kind parameter knd is set by the
c  statement:
...
c          Input Data
c
c  Input parameters are read from unit 1 in the file matfcn.dat
c  The user should create an input file with the lines of data as
c  described. (Note that a subroutine version of matfcn is available
c  where the input parameters and output function values are passed
c  through the call statement).
c
c       line 1:
c
c          lnum   : number of integer values of l given by
c                   0, 1, 2, ..., lnum-1 (integer)
c
c          ioprad : (integer)
...
c                   equal to -1 when q is negative. This is used
c                   to determine q when cm is input.
c
c       line 2:
c
c          q or cm: input size parameter [real(knd)]
c                 : q when icq = 1
...
c       line 3:
c
c          r      : input radial coordinate [real(knd)]
c                   for izxi = 1: equal to the traditional radial
c                   coordinate z ranging from zero to infinity.
c                   for izxi = 2: equal to xi - 1 where xi is the
c                   spheroidal-like radial coordinate equal to cosh(z).
c                   A dummy value, e.g. 1.0d0, should be entered for r
c                   if no radial functions are desired (ioprad=0)
c
c       line 4:
c
c          arg1   : first value for the angle coordinate (in degrees)
c                   [real(knd)]; only read if iopang not equal to 0
c
...
c                   enter the value 1 for narg if iopang = 0
c
...
c        Output Data
c
c  output data is written to five files: fort.20, fort.30,
c  fort.40, fort.50, and fort.60.
c
c   fort.20
c
...
c               mc1s     function of first kind (f17.14). when q is
...
c               imc1de/: exponent of the first derivative of the
c               ims1de   radial function of first kind (i6 for q
c                        positive; i8 for q negative)
c               mc2c/  : characteristic of the radial function
c               ms2c     of second (or third) kind (f17.14). when q
c                        is negative and l is even, mc3c and ms3c are
c                        pure imaginary and the factor i is suppressed
c               imc2e/ : exponent of the radial function of the
c               ims2e    second (or third) kind (i6 for q positive;
c                        i8 for q negative)
c               mc2dc/ : characteristic of the first derivative of the
c               ms2dc    radial function of the second (or third) kind
c                        (f17.14). when q is negative and l is even,
c                        mc3dc and ms3dc are pure imaginary and the
c                        factor i is suppressed
c               imc2de/: exponent of the first derivative of the
c               ims2de   radial function of the second (or third)
...
c                        see statements 620 and 630 in subroutine main).
c                        for l = 0 the value for se is set equal to 0.
c
...
c     to fort.60. Note that this has never been observed using matfcn. 
c
...
c
        use param
c
        real(knd) cm,q,arg1,darg,r,x1,z
c
...
5       kindd=8
        kindq=16
c
c  set the minimum desired accuray minacc to 10 for double
c  precision arithmetic and to 15 for quadruple arithmetic.
c  Minacc for quadruple precision can be changed if desired.
c  See comments above about changing minacc
c
...
c  read input data
        ...
        if(icq.eq.1) read(1,*) q
        if(icq.eq.2) read(1,*) cm
        read(1,*) r
        if(iopang.ne.0) read(1,*) arg1,darg,narg
          if(icq.eq.1) then
          cm=sqrt(2.0e0_knd*abs(q))
          if(q.lt.0.0e0_knd) isq=-1
          end if
          if(icq.eq.2) then
          q=cm*cm/2.0e0_knd
          if(isq.eq.-1) q=-q
...
c
     ...
     1             arg1,darg,narg,maxd,maxj,maxlp,maxn,maxp,maxk,
     2             maxkbp,ndec,nex,ngau,kindd,kindq)
c
        end
c
c
c
     ...
     1                   z,arg1,darg,narg,maxd,maxj,maxlp,maxn,maxp,
     2                   maxk,maxkbp,ndec,nex,ngau,kindd,kindq)
c
...
c
c  real(knd) scalars
        real(knd) api,arg1,a01,asubl,b12,bsubl,cm,cepio2,cedpio2,ce0,
     1            darg,dec,eigaval,eigbval,eigavalp,eigbvalp,eiga1,
     2            eiga2,eiga3,eiga4,eiga5,eigb1,eigb2,eigb3,eigb4,eigb5,
     3            esa,esb,gamma,m1botc,m1bots,mc1c,mc1dc,mc2c,mc2dc,
     4            mc3c,mc3dc,ms1c,ms1dc,ms2c,ms2dc,ms3c,ms3dc,m3bot,pi,
     5            q,rl,sed0,sepio2,sedpio2,sgna,sgnb,wroncc,wroncs,
     6            wront,xi,xb,x1,z
c
...
35          format(1x,' q = ',e40.31)
            ...
            if(icq.eq.2) then
            if(ioprad.ne.0.and.isq.eq.1.and.knd.eq.kindd)
     1         write(20,40) cm
            if(ioprad.ne.0.and.isq.eq.1.and.knd.eq.kindd)
     1         write(40,40) cm
            if(ioprad.ne.0.and.isq.eq.-1.and.knd.eq.kindd)
     1         write(20,50) cm
            if(ioprad.ne.0.and.isq.eq.-1.and.knd.eq.kindd)
     1         write(40,50) cm
            if(ioprad.ne.0.and.isq.eq.1.and.knd.eq.kindq)
     1         write(20,45) cm
            if(ioprad.ne.0.and.isq.eq.1.and.knd.eq.kindq)
     1         write(40,45) cm
            if(ioprad.ne.0.and.isq.eq.-1.and.knd.eq.kindq)
...
55          format(1x,' c = i times',e40.31)
            if(iopang.ne.0.and.isq.eq.1.and.knd.eq.kindd)
     1         write(30,40) cm
            if(iopang.ne.0.and.isq.eq.1.and.knd.eq.kindd)
     1         write(50,40) cm
            if(iopang.ne.0.and.isq.eq.-1.and.knd.eq.kindd)
     1         write(30,50) cm
            if(iopang.ne.0.and.isq.eq.-1.and.knd.eq.kindd)
     1         write(50,50) cm
            if(iopang.ne.0.and.isq.eq.1.and.knd.eq.kindq)
     1         write(30,45) cm
            if(iopang.ne.0.and.isq.eq.1.and.knd.eq.kindq)
     1         write(50,45) cm
            if(iopang.ne.0.and.isq.eq.-1.and.knd.eq.kindq)
...
75        format(1x,/,5x,'Note that when x = 1.0, some of the radial',
     1           ' functions of the second kind can be very small for',
     2           /,5x,'l less than 2c/pi. These are correspondingly',
     3           ' inaccurate. This should not reduce the accuracy',
     4           /,5x,'of solutions to problems where the radial',
     5           ' functions of the second kind appear in a pair with',
     6           /,5x,'the radial functions of the first kind. The',
     7           ' radial functions of the first kind are very',
     8           /,5x,'accurate and dominate the radial functions',
     9           ' of the second kind to the degree the radial',
     A           /,5x,'functions of the second kind are inaccurate.',
     B           ' Note that the Wronskian accuracy overestimates',
     C           /,5x,'the accuracy of the smaller function values',/)
          if(ioprad.ne.0.and.isq.eq.-1.and.x1.eq.0.0q0) write(40,80)
80        format(1x,/,5x,'When x = 1.0 and c is not small and l is',
     1           ' less than about 2c/pi, accuracy of the radial',
     2           /,5x,'functions of the third kind calculated using',
     3           ' the Bessel product expansion is reduced due to',
     4           /,5x,'subtraction errors. These errors are similar',
     5           ' in size to those obtained in computing the angular',
     6           /,5x,'functions ce(0) and sed(0). The associated',
     7           ' radial functions of the first kind are computed',
     8           /,5x,'accurately. When the Wronskian accuracy is less',
     9           ' than the desired minimum accuracy, we use the',
     A           /,5x,'Wronskian to compute an accurate value for the',
     B           ' radial function of the third kind (for sine',
     C           /,5x,'functions). We also do this for the first',
     D           ' derivative of the radial function of the third kind',
     E           /,5x,'(for cosine functions). When the Wronskian',
     F           ' accuracy equals zero digits, the radial functions',
     G           /,5x,'of the third kind (for cosine functions) and',
     H           ' their first derivatives (for sine functions) are',
     I           /,5x,'set  equal to zero.',/)
            do 700 li=1,lnum
            l=li-1
...
280         continue
     ...
     1                          (mc2c*mc1dc)*10.0e0_knd**(imc2e+imc1de)
            naccrc=-int(log10(abs((wroncc-2.0e0_knd/pi)/(2.0e0_knd/pi))
     1              +dec))
...
c
     ...
     1                       ce(jarg),se(jarg),nacca(jarg)
              if(iopang.eq.2) write(30,630) arg(jarg),
     1                       ce(jarg),ced(jarg),se(jarg),
     2                       sed(jarg),nacca(jarg)
620           format(1x,f20.14,5x,e24.15,2x,e24.15,2x,i2)
630           format(1x,f20.14,5x,e24.15,2x,e24.15,2x,/,26x,
     1               e24.15,2x,e24.15,2x,i2)
                if(knd.eq.kindd) then
                if(iopang.eq.1) write(50,640) arg(jarg),
     1                                      ce(jarg),se(jarg)
                if(iopang.eq.2) write(50,650) arg(jarg),
     1                          ce(jarg),ced(jarg),se(jarg),sed(jarg)
640             format(5x,f20.14,' degrees',/,10x,'ce = ',
     1                 e24.15,2x,' se = ',e24.15)
650             format(5x,f20.14,' degrees',/,10x,'ce = ',e24.15,2x,
     1                 ' ced = ',e24.15,/,10x,'se = ',e24.15,2x,
     2                 ' sed = ',e24.15)
                end if
                if(knd.eq.kindq) then
                if(iopang.eq.1) write(50,660) arg(jarg),
     1                     ce(jarg),se(jarg)
                if(iopang.eq.2) write(50,670) arg(jarg),
     1                          ce(jarg),ced(jarg),se(jarg),sed(jarg)
660             format(5x,f20.14,' degrees',/,10x,'ce = ',
     1                 e40.31,2x,' se = ',e40.31)
670             format(5x,f20.14,' degrees',/,10x,'ce = ',e40.31,2x,
     1                 ' ced = ',e40.31,/,10x,'se = ',e40.31,2x,
     2                 ' sed = ',e40.31)
                end if
680           continue
700         continue
...
c  real(knd) scalars and vectors
     ...
     1            dnew,dnewd,dnuma,dnumb,dold,doldd,dterm,fterm,sgna,
     2            sgnb,ssum,sdsum,suma,temp,tempd
...
180     format(3x,'mc1/mc1d numerator converged in ',i5,' terms; ',i5,
     1         ' available; sub. error = ',i3,' digits.')
...
200     format(3x,'mc1/mc1d numerator converged in ',i5,' terms; ',i5,
     1         ' available; fwd series not used; sub. error = ',i3,
     2         ' digits.')
...
150     format(3x,'mc1/mc1d converged in ',i5,' of ',i5,' terms;',
     1         ' sub. errors =',i3,' and',i3,' digits. s = ',i6)
...
70      format(3x,i10,' "a" coefficients; ce(0) converged in ',
     1         i6,' terms; sub. error =',i3,' digits.')
